= RHEL AI and InstructLab: Get a taste of training your own model
...
As you work with InstructLab, you will see the terms *_Skills_* and *_Knowledge_*. What is the difference between Skills and Knowledge? A simple analogy is to think of a skill as teaching the model **how** to do something, while knowledge is teaching the model **information about** something. A skill might teach the model how to produce table data, while a knowledge contribution might teach the model about various table data styles throughout history.

For this workshop, we will focus on the **knowledge** category.
...
There is a temporary UX issue where every time a command is run, the processing time is long due to an underlying process where the instructlab image is duplicated for the user. Running as root is a workaround to the latency, but it is not a requirement to use the product.
...
The response text should indicate that you are running **version 0.21.2**. If you see a different version, please tell your lab proctor.
...
A few things happen during initialization. A taxonomy is generated, a configuration file (`config.yaml`) is created in the `/root/.config/instructlab/` directory, and the appropriate system profile is selected to be used during the fine-tuning process.
...
Once the download completes, enter `ilab model list` into the terminal. You should see the model we just downloaded in the list, in addition to the other preset models on the system.
...
+-----------------------------------+---------------------+---------+
| models/granite-3.1-8b-starter-v1  | 2024-09-24 14:40:57 | 12.6 GB |
+-----------------------------------+---------------------+---------+
----
...
[source,console,role=execute,subs=attributes+]
----
ilab model serve --model-path /root/.cache/instructlab/models/granite-3.1-8b-starter-v1
----
...
[source,console,role=execute,subs=attributes+]
----
ilab model chat --model /root/.cache/instructlab/models/granite-3.1-8b-starter-v1
----
...
NOTE: Docling is now also embedded in RHEL AI as a part of the InstructLab pipeline. This implementation is relatively new, and we currently see value in working on data preparation ahead of beginning the InstructLab process. Therefore, we will be discussing using Docling separately and manually.
...
Data preparation for InstructLab (as for any other type of training) takes care and time. We do not have time to go through this process as thoroughly as we would need to for a PoC or production scenario. Instead of doing the Docling conversion manually during this lab, we will instead move forward with converted .md content to prepare our qna.yaml file. However, we recommend trying Docling out on your own time to see how it works.

Once your documents are converted to .md, the next step in the process is to add the .md file(s) to a git repository. This repository may be private or public. Due to the time constraints of this lab, a repository is provided for you that contains the .md file for our example. 
...
NOTE: The main part of conversion we are not covering is the task of reviewing the .md file to ensure it is correctly formatted, as Docling may not have converted the data perfectly. 

Take a look at the source .md file to gain some familiarity with this format:
...
A proper `qna.yaml` file should have **5-15** context sections and **3** question and answer pairs for each context. Here is an example first section:
...
[#q&a_optional]
==== [OPTIONAL] Practice creating a qna.yaml file

If you're swimming along in the lab and desire extra practice, let's take a stab at creating a full `qna.yaml` file on our own.

We have created a `qna.yaml` template for you to get started with. 

Open this file with your preferred available text editor:

[source,console,role=execute,subs=attributes+]
----
vim ~/fluxdata/fluxmd/qna.yaml
----

You can begin by inserting the example `qna.yaml` section provided in the previous step. 

Now in the second terminal, or in the browser if you'd like to navigate to the GitHub location in a separate tab, open the .md file again. You will need to be able to look at this file as you're creating your `qna.yaml` file.

[source,console,role=execute,subs=attributes+]
----
cat ~/fluxdata/fluxmd/flux.md
----

Use the guidance in the previous section to craft the file. Remember, you are selecting 5 chunks of context and 3 Q&A pairs for each chunk of context. 

It won't be perfect, and that's okay! The goal is to get a feel for the process and to practice.
...
NOTE: The following models have been pre-downloaded to our RHEL AI system due to time constraints. However, we want you to go through the motions a bit and become familiar with the model requirements.
...
You can see that we are already consuming close to 500GB of disk space. Keep in mind, we are using unquantized models and working directly with .safetensors instead of .gguf formatted files. This provides the highest quality models possible at the end of our fine-tuning process.
...
Enter `ilab model list` to look again at our downloaded models. The two LoRA adapters will not display in this command as they are not models, but are each a layer used for our Mixtral model to enhance the SDG process. 
...
You will see all five in this directory (plus a couple of others).

For a more detailed description of the models, please refer to the https://docs.redhat.com/en/documentation/red_hat_enterprise_linux_ai/1.4/html/building_and_maintaining_your_rhel_ai_environment/download_models#download_models[RHEL AI documentation].
...
[source,console,role=execute,subs=attributes+]
----
ilab data generate --max-num-tokens 2000
----

NOTE: We are adding the maximum number of tokens parameter to shorten the processing time. 
...
[source,console]
----
INFO 2025-02-12 01:21:46,537 instructlab.sdg.pipeline:199: Running block: router
INFO 2025-02-12 01:21:48,728 instructlab.sdg.pipeline:199: Running block: SetClassifierValue
INFO 2025-02-12 01:21:48,739 instructlab.sdg.pipeline:199: Running block: duplicate_document_col
INFO 2025-02-12 01:21:48,742 instructlab.sdg.pipeline:199: Running block: gen_detailed_summary
INFO 2025-02-12 01:22:11,049 instructlab.sdg.pipeline:199: Running block: gen_atomic_facts
INFO 2025-02-12 01:22:52,668 instructlab.sdg.pipeline:199: Running block: gen_extractive_summary
INFO 2025-02-12 01:23:09,756 instructlab.sdg.pipeline:199: Running block: flatten_summary_columns
INFO 2025-02-12 01:23:09,770 instructlab.sdg.pipeline:199: Running block: rename_to_document_column
INFO 2025-02-12 01:23:09,777 instructlab.sdg.pipeline:199: Running block: knowledge generation
----

This should only take a couple of minutes. Take a break, meditate, etc. The SDG is spending the most time on preset skills data in the system, not on our knowledge data.
...
[source,console]
----
INFO 2025-02-12 01:33:20,986 instructlab.sdg.generate_data:485: Generation took 743.64s
INFO 2025-02-12 01:33:27,611 instructlab.model.backends.vllm:475: Waiting for GPU VRAM reclamation...
----
...
[source,console, role=execute, subs=attributes+]
----
ilab model train --data-path  /root/.local/share/instructlab/datasets/knowledge_train_msgs_[timestamp].jsonl --num-epochs 1 --device=cuda --model-path ~/.cache/instructlab/models/granite-3.1-8b-starter-v1
----

This will take a few minutes. Take a look at the CLI output and get a sense of how it works. When complete, you should see the following which indicates that the training is complete.

[source,console]
----
Saving training state: {'current_epoch': 0, 'samples_seen': 372}
Model state saved in: /root/.local/share/instructlab/checkpoints/full_state/epoch_0
Epoch 0: 100%|██████████| 6/6 [02:50<00:00, 28.36s/it]
...
=== [OPTIONAL] Running a One Epoch Trained Model
...
NOTE: Please change the samples folder to your appropriate model folder.

[source,console, role=execute, subs=attributes+]
----
ilab model serve --model-path /root/.local/share/instructlab/checkpoints/hf_format/samples_###
----
...
[source,console, role=execute, subs=attributes+]
----
ilab model chat -gm --model /root/.local/share/instructlab/checkpoints/hf_format/samples_###
----
...
* **Phase 1**: We take the starter model, and fine-tune it with the synthetic knowledge dataset produced from SDG. This training process outputs a model ‘checkpoint’ after each epoch of training. The checkpoints are evaluated by Prometheus using standard LLM benchmarks. The best model checkpoint goes to Phase 2.
...
Test it out with the same questions.

NOTE: This model was tuned with an older version of our product, with a dataset that is too small. This does not adequately reflect performance but is enough for a simple demo.
